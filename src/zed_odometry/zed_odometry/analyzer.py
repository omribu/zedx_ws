...
DISTANCE = 0.0  # meters — positive = forward, negative = backward
...
class Analyzer:
    ...
    def error_calculation(self, x_start, y_start, x_stop_world, y_stop_world):
        ...
        # Use the extremal-displacement sample as the endpoint rather than the
        # last sample. The bag may include the return journey after stop.png was
        # taken, which brings the odom back to near-zero and gives a false ~20 m
        # error. The turnaround point is the sample with max (forward) or min
        # (backward) x-displacement.
        # For DISTANCE==0 (circular / return-to-origin experiments), argmax would
        # pick the midpoint of the loop rather than the true end — use last sample.
        if DISTANCE == 0:
            self._endpoint_idx = len(self.x_positions) - 1
        elif DISTANCE > 0:
            self._endpoint_idx = int(np.argmax(self.x_positions))
        else:
            self._endpoint_idx = int(np.argmin(self.x_positions))

        zed_last_odom_x = self.x_positions[self._endpoint_idx]
        zed_last_odom_y = self.y_positions[self._endpoint_idx]
        ...
        print(f"\nOdometry endpoint (idx={self._endpoint_idx}): x={zed_last_odom_x:.4f} m, y={zed_last_odom_y:.4f} m")
    ...
    def plot_trajectory(self, x_start, y_start, x_stop_world, y_stop_world, error, save_path=None):
        ...
        # Odometry end point — use the same extremal-displacement index as error_calculation
        endpoint_idx = getattr(self, '_endpoint_idx', -1)
        x_last = self.x_positions[endpoint_idx]
        y_last = self.y_positions[endpoint_idx]
        ...
        pct = f'{(error / abs(DISTANCE) * 100):.2f}%' if DISTANCE != 0 else 'N/A'
        textstr += f'Error percentage: {pct}'
...
def main(args=None):
    ...
    if odom_analyzer.read_bag(bag_path):
        ...
        save_results_csv(
            ...
            error=error,
            odom_end_x=odom_analyzer.x_positions[odom_analyzer._endpoint_idx],
            odom_end_y=odom_analyzer.y_positions[odom_analyzer._endpoint_idx],
            gt_start_x=x_start,
            gt_start_y=y_start,
...
# # REPLACE PATH for specific experiment
# NAME = "9"
# EXP_TYPE = "1.0m_s/20meter/" 
# DATE = "3_6_26"
# BAG_PATH = "/home/volcani/workspaces/zedx_ws/zed_odom_recordings/" + DATE + "/" + EXP_TYPE + "/" + NAME + "/"

# DISTANCE = 20.0  # meters — positive = forward, negative = backward
