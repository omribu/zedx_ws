...
# # This code analyzes experiment data - odometry data from a rosbag and images of the initial position and last position
# # first 3 images pop up - by clicking first the pixel of the origin coordinate frame, than on the x axis direction 
# # and lastly on the target point which is the projection of the zed camera on the plane.
# # we do the same for the last image which pops up 3 images 
# # after this the code calculates the world coordinates relative to the origin point and calculates 
...
# BAG_PATH = "/home/volcani/zedx_ws/zed_odom_recordings/21_1_2026/evning/0.5m_s/"
# NAME = "1_forward_cam_horizon"   

# DISTANCE = 5.0  # meters  (ground truth distance traveled in X direction)




# H = 0.445 # Camera height in meters
# PIXEL_QUERY = [624, 156]
...
#         print("➡ Click START image: 1) Origin point 2) X-axis direction point")
#         for i in range(2):
#             pixel = PixelValueExtractor.get_pixel_value(
#                 image_start, window_name="START IMAGE"
#             )
...
#         print("➡ Click STOP image: 1) Origin point 2) X-axis direction point")
#         for i in range(2):
#             pixel = PixelValueExtractor.get_pixel_value(
#                 image_stop, window_name="STOP IMAGE"
#             )
...
#     def error_calculation(self, x_start_local, y_start_local, x_stop_local, y_stop_local):
#         """Calculate error between odometry and image-based world coordinates."""
        
#         # FIXED: Ground truth positions in global world frame
#         # Start position is at (0, 0) + local offset from start image
#         world_x_start = 0.0 + x_start_local
#         world_y_start = 0.0 + y_start_local
        
#         # Stop position is at (DISTANCE, 0) + local offset from stop image
#         world_x_stop = DISTANCE + x_stop_local
#         world_y_stop = 0.0 + y_stop_local
        
#         print(f"\n=== Ground Truth Positions ===")
#         print(f"Start (world frame): x={world_x_start:.4f} m, y={world_y_start:.4f} m")
#         print(f"Stop (world frame):  x={world_x_stop:.4f} m, y={world_y_stop:.4f} m")
        
#         # Get initial and final odometry values
#         odom_x_initial = self.x_positions[0]
#         odom_y_initial = self.y_positions[0]
#         odom_x_final = self.x_positions[-1]
#         odom_y_final = self.y_positions[-1]
        
#         print(f"\n=== Raw Odometry Data ===")
#         print(f"Initial odometry: x={odom_x_initial:.4f} m, y={odom_y_initial:.4f} m")
#         print(f"Final odometry: x={odom_x_final:.4f} m, y={odom_y_final:.4f} m")
#         print(f"Odometry displacement: dx={odom_x_final - odom_x_initial:.4f} m, dy={odom_y_final - odom_y_initial:.4f} m")
        
#         # Transform odometry to world frame:
#         # The odometry gives relative displacement from its start
#         # We need to align it so it starts at world_x_start, world_y_start
#         self.x_positions = [(x - odom_x_initial) + world_x_start for x in self.x_positions]
#         self.y_positions = [(y - odom_y_initial) + world_y_start for y in self.y_positions]

#         # Odometry final position in world frame
#         zed_last_odom_x = self.x_positions[-1] 
#         zed_last_odom_y = self.y_positions[-1] 

#         # Calculate error
#         delta_x = zed_last_odom_x - world_x_stop
#         delta_y = zed_last_odom_y - world_y_stop
#         total_error = np.sqrt(delta_x**2 + delta_y**2)

#         print(f"\n=== Odometry vs Ground Truth ===")
#         print(f"Odometry final position (world frame): x={zed_last_odom_x:.4f} m, y={zed_last_odom_y:.4f} m")
#         print(f"Ground truth final position: x={world_x_stop:.4f} m, y={world_y_stop:.4f} m")
#         print(f"Error: Δx={delta_x:.4f} m, Δy={delta_y:.4f} m, Total Error={total_error:.4f} m")

#         return total_error, world_x_start, world_y_start, world_x_stop, world_y_stop
...
#         # Plot reference straight line from start to stop
#         ax.plot([x_start, x_stop], [y_start, y_stop], 
#                 'g--', label='Ground Truth Path', linewidth=2)
        
#         # Plot start and stop points
#         ax.plot(x_start, y_start, 'go', markersize=12, label='Start Position', markeredgecolor='black', markeredgewidth=1.5)
#         ax.plot(x_stop, y_stop, 'ro', markersize=12, label='Ground Truth End Position', markeredgecolor='black', markeredgewidth=1.5)
...
#         ax.set_xlabel('X Position (m)', fontsize=12)
#         ax.set_ylabel('Y Position (m)', fontsize=12)
#         ax.set_title('Odometry Trajectory with Ground Truth Path and Error', fontsize=14, fontweight='bold')
#         ax.axis('equal')
#         ax.grid(True, alpha=0.3)
#         ax.legend(loc='best', fontsize=10)
        
#         # Add text box with statistics
#         distance_traveled = np.sqrt((x_stop - x_start)**2 + (y_stop - y_start)**2)
#         textstr = f'Ground truth distance: {distance_traveled:.4f} m\n'
#         textstr += f'Total error: {error:.4f} m\n'
#         textstr += f'Error percentage: {(error / distance_traveled * 100):.2f}%'
#         props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
#         ax.text(0.02, 0.98, textstr, transform=ax.transAxes, fontsize=10,
#                 verticalalignment='top', bbox=props)
...
#         print("\n=== Processing START image ===")
#         x_start_local, y_start_local = odom_analyzer.pixel_to_world(
#             start_pixels[0],    # origin pixel
#             start_pixels[1],    # x direction pixel
#             odom_analyzer.K,
#             odom_analyzer.H
...
#         print(f"Start local coordinates (relative to clicked origin): x={x_start_local:.4f} m, y={y_start_local:.4f} m")
#         print("\n=== Processing STOP image ===")
...
#         print(f"Stop local coordinates (relative to clicked origin): x={x_stop_local:.4f} m, y={y_stop_local:.4f} m")

#         # Error calculation (now handles coordinate transformation internally)
#         error, world_x_start, world_y_start, world_x_stop, world_y_stop = odom_analyzer.error_calculation(
#             x_start_local, y_start_local, x_stop_local, y_stop_local
#         )

#         # Plot results with corrected coordinates
#         save_path = os.path.join(os.path.dirname(bag_path), NAME + "_corrected.png")
#         odom_analyzer.plot_trajectory(world_x_start, world_y_start, world_x_stop, world_y_stop, error, save_path)
